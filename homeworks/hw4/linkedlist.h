...
#define LinkedList__append(type, list, v) {linkedlistNodeTypeName(type)* UNIQUE(current) = list.first; while (UNIQUE(current) && UNIQUE(current)->next) {printf("here\n"); UNIQUE(current) = UNIQUE(current)->next;}; linkedlistNodeTypeName(type)* UNIQUE(next) = LinkedList__createNodePtr(type); UNIQUE(next)->value = v; if (list.first) {UNIQUE(current)->next = UNIQUE(next);} else {list.first = UNIQUE(next);};}
...
#define LinkedList__length(type, list) {linkedlistNodeTypeName(type)* UNIQUE(current) = list.first; int toReturn = 0; }
