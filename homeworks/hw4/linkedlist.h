...
#define LinkedList__foreach(type, varname, list) if (list) {type varname = list->value; for (linkedlistTypeName(type) UNIQUE_NAME = list; UNIQUE_NAME; ({UNIQUE_NAME = UNIQUE_NAME->next; varname = (UNIQUE_NAME)? UNIQUE_NAME->value : varname;}))}
